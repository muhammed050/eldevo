...
export const unsupportedToolSlugs = new Set<string>([
  ...
  "image-to-base64", "base64-to-image", "image-resizer", "image-cropper", "image-compressor",
  "jpg-to-png", "png-to-jpg", "webp-converter", "favicon-generator", "css-formatter",
]);
...
function definition(item: ToolMeta, kind: "tool" | "converter"): ToolDefinition {
  return { ...item, status: "active", kind, href: `/${kind === "converter" ? "converters" : "tools"}/${item.slug}/`, execute: (input) => executeActiveTool(item.slug, input), validate: validateInput };
}
...
export function searchTools(query: string, category = "All"): ToolDefinition[] {
  ...
  return toolEntries.filter((tool) => category === "All" || tool.category === category).map((tool) => {
    ...
    return { tool, score };
  }).filter(({ score }) => !q || score > 0).sort((a, b) => b.score - a.score || a.tool.title.localeCompare(b.tool.title)).map(({ tool }) => tool);
}
