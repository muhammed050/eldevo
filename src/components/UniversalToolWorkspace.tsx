...
import { executeActiveTool } from "@/lib/tools/executor";
...
const samples: Record<string, string> = {
  ...
  "number-base-converter": "255 10 16",
  "html-formatter": "<div><h1>Hello</h1><p>World</p></div>",
  "text-case-converter": "camel: hello world from eldevo",
...
export function UniversalToolWorkspace({ slug }: { slug: string }) {
  ...
  useEffect(() => { setInput(samples[slug] ?? ""); setOutput(""); setError(""); }, [slug]);
  const run = useCallback(async () => {
    setBusy(true); setOutput(""); setError("");
    try { setOutput(await executeActiveTool(slug, input)); }
    catch (e) { setError(e instanceof Error ? e.message : "Unable to process this input."); }
    finally { setBusy(false); }
  }, [input, slug]);
  const copy = async () => { if (output) await navigator.clipboard.writeText(output); };
  const download = () => { if (!output) return; const url = URL.createObjectURL(new Blob([output], { type: "text/plain;charset=utf-8" })); const a = document.createElement("a"); a.href = url; a.download = `${slug}-result.txt`; a.click(); URL.revokeObjectURL(url); };
  return <section className="rounded-2xl border border-slate-800 bg-slate-950 p-4 sm:p-6">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><span className="font-mono text-xs uppercase tracking-[.16em] text-cyan-300">Client-side • {slug}</span><button onClick={run} disabled={busy || !input.trim()} className="rounded-lg bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950 disabled:opacity-50">{busy ? "Processing…" : "Run Tool"}</button></div>
