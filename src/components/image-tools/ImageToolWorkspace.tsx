...
export function ImageToolWorkspace({ operation }: Props) {
  ...
  const choose = useCallback((next: File | undefined) => {
    ...
    image.onerror = () => setError("The browser could not read this image.");
    image.src = url;
  ...
  const reset = () => {
    setRotation(0); setFlipX(false); setFlipY(false); setBrightness(100); setContrast(100);
    setSaturation(100); setGrayscale(0); setBlur(0); setInvert(0); setSepia(0); setHue(0); setZoom(100);
    setCropEnabled(false); setLockAspect(false); setError("");
    if (file && preview) {
      const image = new Image();
  ...
  const setCropPreset = (ratio: number | null) => {
    ...
    if (h > height) { h = height; w = Math.round(h * ratio); }
    setCropX(Math.max(0, Math.round((width - w) / 2)));
    setCropY(Math.max(0, Math.round((height - h) / 2)));
    setCropWidth(w); setCropHeight(h); setCropEnabled(true);
  };
  ...
  const run = async () => {
    ...
    try {
      const blob = await processImage(file, {
        operation, width, height, quality: quality / 100, format, rotation, flipX, flipY,
        brightness, contrast, saturation, grayscale, blur, invert, sepia, hue,
        ...(operation === "editor" && cropEnabled ? { x: cropX, y: cropY, cropWidth, cropHeight } : {}),
      });
      if (resultUrl) URL.revokeObjectURL(resultUrl);
      setResultUrl(URL.createObjectURL(blob)); setResultSize(blob.size);
    } catch (e) {
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-950 p-4 sm:p-6">
      ...
      >
        ...
        <span className="text-sm font-semibold text-slate-200">{dragging ? "Release to load image" : "Drop an image here or click to upload"}</span>
        <span className="mt-2 text-xs text-slate-500">PNG, JPG or WebP · up to 25 MB · paste an image from clipboard</span>
      </label>

      {file && (
        <>
          <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="overflow-auto rounded-xl border border-slate-800 bg-slate-900 p-3">
              <div className="flex min-h-[420px] items-center justify-center overflow-auto">
                <img src={preview} alt="Selected image preview" className="max-h-[560px] max-w-full object-contain" style={{ filter: previewFilter, transform: `${flipX ? "scaleX(-1)" : ""} ${flipY ? "scaleY(-1)" : ""} rotate(${rotation}deg) scale(${zoom / 100})` }} />
              </div>
              <div className="mt-3 flex items-center gap-3"><span className="text-xs text-slate-500">Preview zoom</span><input type="range" min="50" max="200" value={zoom} onChange={(e) => setZoom(Number(e.target.value))} className="w-full" /><span className="w-12 text-right font-mono text-xs text-slate-500">{zoom}%</span></div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => rotateBy(-90)} className="btn">↶ Rotate left</button><button type="button" onClick={() => rotateBy(90)} className="btn">↷ Rotate right</button>
                <button type="button" onClick={() => setFlipX((v) => !v)} className={`btn ${flipX ? "active" : ""}`}>↔ Flip H</button><button type="button" onClick={() => setFlipY((v) => !v)} className={`btn ${flipY ? "active" : ""}`}>↕ Flip V</button>
              </div>
              ...
              {cropEnabled && <div className="grid grid-cols-2 gap-3"><Field label="Crop X" value={cropX} setValue={setCropX} min={0} /><Field label="Crop Y" value={cropY} setValue={setCropY} min={0} /><Field label="Crop width" value={cropWidth} setValue={(v) => { setCropWidth(v); if (lockAspect) setCropHeight(Math.max(1, Math.round(v * cropHeight / Math.max(1, cropWidth)))); }} min={1} /><Field label="Crop height" value={cropHeight} setValue={setCropHeight} min={1} /></div>}
