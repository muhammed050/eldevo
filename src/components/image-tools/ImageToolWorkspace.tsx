...
export function ImageToolWorkspace({ operation }: Props) {
  ...
  const choose = (next: File | undefined) => {
    if (!next) return;
    setError(""); setResultUrl(""); setResultSize(0); setFile(next);
    if (preview) URL.revokeObjectURL(preview);
    setPreview(URL.createObjectURL(next));
    const image = new Image();
    image.onload = () => { setWidth(image.naturalWidth); setHeight(image.naturalHeight); };
  ...
  const run = async () => {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to process this image."); }
    finally { setBusy(false); }
  };
  const download = () => { if (!resultUrl) return; const a = document.createElement("a"); a.href = resultUrl; a.download = `eldevo-${operation}.${format === "image/jpeg" ? "jpg" : format.slice(6)}`; a.click(); };
