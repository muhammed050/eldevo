...
export function ToolPageContent({
  ...
  const isJsonTool = jsonToolSlugs.has(meta.slug);
  ...
  return (
    <article className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      <nav aria-label="Breadcrumb" className="mb-5 text-xs text-slate-500">
        ...
        <Link href={meta.kind === "converter" ? "/converters/" : "/tools/"} className="hover:text-cyan-400">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
        <div className="min-w-0">
          ...
          <div className="mt-5">
            {isJsonTool ? (
              <JsonToolWorkspace slug={meta.slug} />
            ) : isStrategic ? (
            ...
            <ol className="mt-4 grid gap-3">
              {meta.usageSteps.map((step, index) => (
                <li key={step} className="flex gap-3 text-sm leading-6 text-slate-400">
                  <span className="grid size-6 shrink-0 place-items-center rounded-full bg-slate-800 font-mono text-xs text-cyan-300">{index + 1}</span>
                  {step}
                </li>
              ))}
            </ol>
          ...
          <section className="mt-12">
            ...
            <ul className="mt-4 grid gap-2 text-sm leading-6 text-slate-400">
              {meta.features.map((feature) => <li key={feature}>✓ {feature}</li>)}
            </ul>
            ...
            <div className="mt-4 divide-y divide-slate-800 rounded-xl border border-slate-800 bg-slate-900">
              {meta.faqs.map((item) => (
                <details key={item.q} className="p-5">
                  <summary className="cursor-pointer text-sm font-medium text-slate-200">{item.q}</summary>
          ...
          {related.length > 0 && (
            <section className="mt-12">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {related.map((item) => (
                  <Link key={item.href} href={item.href} className="rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm capitalize text-slate-300 transition hover:border-cyan-500/50 hover:text-cyan-300">
                    {item.label}
                  </Link>
                ))}
              </div>
          ...
          <section className="mt-12 rounded-xl border border-slate-800 bg-slate-900/60 p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-400">
              ElDevo is designed around browser-based processing. Unless a tool explicitly states otherwise, your input is transformed on your device rather than uploaded to an ElDevo server.
            </p>
          </section>
        </div>
        <aside className="hidden xl:block">
          <div className="sticky top-20"><AdSlot label="Advertisement" /></div>
        </aside>
      </div>
    </article>
  );
...
function CodeBlock({ label, value }: { label: string; value: string }) {
  return (
    <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
      <div className="border-b border-slate-800 px-3 py-2 font-mono text-[10px] uppercase tracking-[.18em] text-slate-500">{label}</div>
