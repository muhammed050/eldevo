...
import { StrategicToolWorkspace } from "@/components/StrategicToolWorkspace";
...
export function ToolPageContent({ meta, path }: { meta: ToolMeta & { kind: "tool" | "converter" }; path: string }) {
  ...
  return <article className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]"><div className="min-w-0">
      <PrivacyBanner />
      <div className="mt-5">{isStrategic ? <StrategicToolWorkspace slug={meta.slug} /> : <ToolWorkspace slug={meta.slug} />}</div>
      ...
      <section className="mt-12"><h2 className="text-xl font-semibold">Frequently Asked Questions</h2><div className="mt-4 divide-y divide-slate-800 rounded-xl border border-slate-800 bg-slate-900">{meta.faqs.map(item=><details key={item.q} className="p-5"><summary className="cursor-pointer text-sm font-medium text-slate-200">{item.q}</summary><p className="mt-3 text-sm leading-6 text-slate-400">{item.a}</p></details>)}</div></section>
      {related.length>0&&<section className="mt-12"><h2 className="text-xl font-semibold">Related Tools</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{related.map(item=><Link key={item.href} href={item.href} className="rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm capitalize text-slate-300 transition hover:border-cyan-500/50 hover:text-cyan-300">{item.label}</Link>)}</div></section>}
      <section className="mt-12 rounded-xl border border-slate-800 bg-slate-900/60 p-5"><h2 className="text-lg font-semibold">Privacy-first processing</h2><p className="mt-2 text-sm leading-6 text-slate-400">ElDevo is designed around browser-based processing. Unless a tool explicitly states otherwise, your input is transformed on your device rather than uploaded to an ElDevo server.</p></section>
    </div><aside className="hidden xl:block"><div className="sticky top-20"><AdSlot label="Advertisement"/></div></aside></div>
  </article>;
}
function CodeBlock({label,value}:{label:string;value:string}){return <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900"><div className="border-b border-slate-800 px-3 py-2 font-mono text-[10px] uppercase tracking-[.18em] text-slate-500">{label}</div><pre className="overflow-x-auto p-4 font-mono text-xs leading-6 text-slate-300">{value}</pre></div>}
