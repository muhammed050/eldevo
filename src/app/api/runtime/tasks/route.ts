...
const schema = z.object({ agentId: z.string().uuid(), goal: z.string().min(3).max(10000), budgetCents: z.number().int().positive().optional(), idempotencyKey: z.string().min(8).max(128).optional(), metadata: z.record(z.string(), z.unknown()).optional() });
...
export async function POST(request: Request) {
  try {
    ...
    if (body.idempotencyKey) {
      const { data: existing } = await supabase.from("tasks").select("id,status,output,error,metadata").eq("organization_id", membership.organization_id).eq("idempotency_key", body.idempotencyKey).maybeSingle();
