...
const schema = z.object({ agentId: z.string().uuid(), goal: z.string().min(3).max(10000), budgetCents: z.number().int().positive().optional(), metadata: z.record(z.string(), z.unknown()).optional() });
...
export async function POST(request: Request) {
  try {
    ...
    const result = await executeTask({ organizationId: membership.organization_id, goal: body.goal, agentId: agent.id, budgetCents: body.budgetCents, metadata: body.metadata }, agent, user.id);
