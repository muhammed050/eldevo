...
export async function POST(request: Request) {
  try {
    ...
    const organizationId = resolveTaskOrganization(dbAgent.organization_id, membership.organization_id);
    if (dbAgent.status !== "active") return NextResponse.json({ error: "Agent is not active" }, { status: 409 });

    if (body.idempotencyKey) {
      const { data: existing } = await supabase.from("tasks").select("id,status,output,error").eq("organization_id", organizationId).eq("idempotency_key", body.idempotencyKey).maybeSingle();
