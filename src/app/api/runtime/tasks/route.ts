...
export async function POST(request: Request) {
  try {
    ...
    const { data: membership, error: membershipError } = await supabase
      ...
      .eq("user_id", user.id)
      .eq("organization_id", dbAgent.organization_id)
      .maybeSingle();
    ...
    if (body.idempotencyKey) {
      const { data: existing } = await supabase.from("tasks").select("id,status,output,error").eq("organization_id", dbAgent.organization_id).eq("idempotency_key", body.idempotencyKey).maybeSingle();
    ...
    const result = await executeTask({ organizationId: dbAgent.organization_id, goal: body.goal, agentId: agent.id, budgetCents: body.budgetCents, idempotencyKey: body.idempotencyKey, metadata: body.metadata ?? {} }, agent, user.id, { enqueue: body.executionMode === "queued" });
    await supabase.from("audit_logs").insert({ organization_id: dbAgent.organization_id, user_id: user.id, action: body.executionMode === "queued" ? "task.enqueue" : "task.execute", resource_type: "task", resource_id: result.taskId, metadata: { agent_id: agent.id, status: result.status, execution_mode: body.executionMode, usage: result.usage } });
