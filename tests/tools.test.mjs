...
describe("active tool smoke suite", () => {
  for (const tool of activeTools) test(`${tool.slug} executes a real implementation`, async () => {
    const input = fixture(tool.slug, tool.codeExample.input);
    try { const result = await tool.execute(input); assert.equal(typeof result, "string"); assert.ok(result.length > 0, "tool returned an empty result"); assert.notEqual(result, "Processed by ElDevo locally"); }
    catch (error) { assert.equal(error instanceof ToolUnsupportedError, false, `active tool ${tool.slug} is unsupported`); throw error; }
  });
});
